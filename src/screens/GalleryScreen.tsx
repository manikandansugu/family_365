  ...
  const GalleryScreen = () => {
    ...
    const [currentImage, setCurrentImage] = useState(null);
      const fetchImageData = async () => {
        try {
          ...
          const metadataResponse = await API_INSTANCE.get(`v1/storage/fetch-all-files-info-from-storage?id=${user?.orphanageId}&uploaderType=ORPHANAGE`)
          ...
          const imagePromises = metadataJson.data.map(async (item: { filePath: string | number | boolean; }) => {
            const imagePath = encodeURIComponent(item.filePath); // Ensure filePath is URL-encoded
            console.log(imagePath)
            const imageResponse = await  API_INSTANCE.get(`v1/storage/fetch-any-file-from-storage-by-path?filePath=${imagePath}`, { responseType: 'arraybuffer' })
    ...
    return (
      ...
      <ScrollView style={styles.container}>
             <Text style={styles.mainHeading}>Welcome to the Gallery</Text>
